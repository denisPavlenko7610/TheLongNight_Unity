...
namespace TLN.Gameplay.Saves
{
    ...
    public sealed class PersistentWorldEntity : MonoBehaviour
    {
        ...
        public bool IsSceneObject =>
            string.IsNullOrWhiteSpace(_prefabId);
            ...
            if (IsSceneObject)
            {
                if (string.IsNullOrWhiteSpace(_id))
                {
                    TLNLogger.LogWarning(
                        "Scene persistent entity has empty id.",
                        this);
                }
        ...
        public void InitializeRuntimeEntity(string id)
        {
            ...
            {
                TLNLogger.LogWarning(
                    "Cannot initialize runtime id for scene entity.",
                    this);
        ...
        public WorldEntitySaveData CaptureState()
        {
            WorldEntitySaveData data = new WorldEntitySaveData
            {
                ...
                position = Vector3SaveData.FromVector3(
                    transform.position),
                rotation = QuaternionSaveData.FromQuaternion(
                    transform.rotation)
            };
