...
namespace TLN.Editor.Saves
{
    public static class SaveSetupTools
    {
        private const string ItemCatalogPath =
            "Assets/_Project/Config/Saves/ItemCatalog.asset";

        private const string WorldPrefabCatalogPath =
            "Assets/_Project/Config/Saves/WorldPrefabCatalog.asset";
        ...
        private static void AddMissingPersistentEntitiesToSceneSaveables()
        {
            ...
            for (int i = 0; i < behaviours.Length; i++)
            {
                ...
                if (behaviour.TryGetComponent<PersistentWorldEntity>(
                        out _))
                {
                    continue;
                }

                Undo.AddComponent<PersistentWorldEntity>(
            ...
            Debug.Log(
                $"[TLN Saves] Added PersistentWorldEntity components: {addedCount}");
        }

        private static void FixScenePersistentIds()
        {
            PersistentWorldEntity[] entities =
                UnityEngine.Object.FindObjectsByType<PersistentWorldEntity>(
                    FindObjectsInactive.Include,
                    FindObjectsSortMode.None);
            ...
            for (int i = 0; i < entities.Length; i++)
            {
                ...
                SerializedObject serializedObject =
                    new SerializedObject(entity);

                SerializedProperty idProperty =
                    serializedObject.FindProperty("_id");

                SerializedProperty prefabIdProperty =
                    serializedObject.FindProperty("_prefabId");

                if (idProperty == null ||
                    prefabIdProperty == null)
                {
                    continue;
                }
            ...
            Debug.Log(
                $"[TLN Saves] Fixed scene persistent ids: {fixedCount}");
        }
        ...
        private static void ConfigurePrefabPersistentIds()
        {
            ...
            for (int i = 0; i < prefabGuids.Length; i++)
            {
                ...
                PersistentWorldEntity entity =
                    prefab.GetComponent<PersistentWorldEntity>();
            ...
            Debug.Log(
                $"[TLN Saves] Configured prefab persistent ids: {configuredCount}");
        }

        private static void RebuildItemCatalog()
        {
            ItemCatalog catalog =
                FindOrCreateAsset<ItemCatalog>(
                    ItemCatalogPath);
            ...
            Debug.Log(
                $"[TLN Saves] ItemCatalog rebuilt. Items: {items.Length}");
        }

        private static void RebuildWorldPrefabCatalog()
        {
            WorldPrefabCatalog catalog =
                FindOrCreateAsset<WorldPrefabCatalog>(
                    WorldPrefabCatalogPath);

            List<WorldPrefabCatalog.EntryDraft> entries =
                new List<WorldPrefabCatalog.EntryDraft>();
            ...
            for (int i = 0; i < prefabGuids.Length; i++)
            {
                ...
                {
                    Debug.LogWarning(
                        $"[TLN Saves] Duplicate prefab id: {prefabId}. Prefab: {path}",
                        prefab);
                ...
                entries.Add(
                    new WorldPrefabCatalog.EntryDraft(
                        prefabId,
                        prefab));
            }
            ...
            Debug.Log(
                $"[TLN Saves] WorldPrefabCatalog rebuilt. Prefabs: {entries.Count}");
        }
        ...
        private static void ValidateCurrentSceneSaveSetup()
        {
            ...
            Dictionary<string, PersistentWorldEntity> ids =
                new Dictionary<string, PersistentWorldEntity>();
            ...
            for (int i = 0; i < entities.Length; i++)
            {
                ...
                {
                    Debug.LogError(
                        "[TLN Saves] Scene persistent entity has empty Id.",
                        entity);
                ...
                if (ids.TryGetValue(
                        entity.Id,
                        out PersistentWorldEntity duplicate))
                {
                    Debug.LogError(
                        $"[TLN Saves] Duplicate scene persistent Id: {entity.Id}",
                        entity);

                    Debug.LogError(
                        $"[TLN Saves] First object with duplicate Id: {entity.Id}",
                        duplicate);
            ...
            {
                Debug.Log(
                    $"[TLN Saves] Scene save setup is valid. Persistent entities: {entities.Length}");
            }
            else
            {
                Debug.LogError(
                    $"[TLN Saves] Scene save setup has errors: {errorCount}");
            }
        }
        ...
        private static T FindOrCreateAsset<T>(
            string assetPath)
